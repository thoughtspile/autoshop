import './Leaflet.Sleep.js';
import adapter from './adapter';

if (typeof L !== 'undefined') {
  const mymap = L.map('mapid').setView([55.7558, 37.6173], 8);
  ...
  adapter.get('/api/shops').then((res) => {
    res.shops.forEach(function(shop) {
      ...
      const marker = L.marker([shop.lat, shop.lon]).addTo(mymap);
      const needPopup = document.getElementsByClassName('intro__promo-copy').length !== 0;
      if (needPopup) {
